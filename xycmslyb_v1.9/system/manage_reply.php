...
<div class="mainbox">
  ...
  <form name="addform" id="addform" action="?del=checkbox" method="post">
   <?php
   if($x_id=='' || $x_id==0){
	   $sql="select id from xycms_book_reply";
   }else{
	   $sql="select id from xycms_book_reply where b_id=$x_id";
   }
   ...
   if($x_m=='search' and $x_keys!=""){
	$xy_sql="select * from xycms_book_reply where id in($xyids) and title like '%$x_keys%' order by id desc limit $page->firstcount,$page->displaypg";
   }else{
	$xy_sql="select * from xycms_book_reply where id in($xyids) order by id desc limit $page->firstcount,$page->displaypg";
   }
...
if($_GET['del']=='checkbox'){
	...
	if(empty($ids)){
		...
		switch($cz_lx){
			case 1:
			$sql="DELETE FROM xycms_book_reply where id in ($id)";
			$db->query($sql);
		} 
