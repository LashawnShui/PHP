...
if($x_m=='search' and $x_keys!=""){
	$xy_sql="select * from xycms_book_class where title like '%$x_keys%' order by c_order asc";
}else{
	$xy_sql="select * from xycms_book_class order by c_order asc";
}
...
if($_GET['del']=='checkbox'){
	...
	if(empty($ids)){
		...
		switch($cz_lx){
			case 1:
			$sql="DELETE FROM xycms_book_class where id in ($id)";
			break;
		}
		$db->query($sql);  
