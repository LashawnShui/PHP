      ...
      <table width="100%" cellspacing="0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td align="center"><a href="hf_book.php?id=<?php echo $row['id'];?>" class="a_btn">新回复</a> | <a href="edit_book.php?id=<?php echo $row['id'];?>" class="a_btn">查看/编辑</a> | <a href="javascript:if(ask('警告：删除后将不可恢复，可能造成意想不到后果？')) location.href='del_book.php?id=<?php echo $row['id'];?>';" onClick="delcfm();" class="a_btn">删除</a></td>
          </tr>
          <?php
		  }
...
function getcategoryname($catid){
	global $db;
	$sql="select title from xycms_book_class where id=$catid";
	$result =$db->query($sql);
...
function getnums($catid){
	global $db;
	$sql="select id from xycms_book_reply where b_id=$catid";
	$result =$db->query($sql);
	if($result && mysql_affected_rows()){
...
if($_GET['del']=='checkbox'){
	...
	if(empty($ids)){
		...
		switch($cz_lx){
			case 1:
			$sql="UPDATE xycms_book set is_view=1 where id in ($id)";
			break;
			case 2:
			$sql="UPDATE xycms_book set is_view=0 where id in ($id)";
			break;
			case 3:
			$sql="DELETE FROM xycms_book where id in ($id)";
			break;
		}
		$db->query($sql);
