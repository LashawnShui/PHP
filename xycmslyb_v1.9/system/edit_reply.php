...
if($_GET["act"]==ok){
	...
	$db->update("xycms_book_reply", $siteinfo,"id=$c_id");
	//$db->close();
...
$e_rs=$db->get_one("select * from xycms_book_reply where id=$sxid",MYSQL_ASSOC);
$bid=$e_rs['id'];
$b_id=$e_rs['b_id'];
$s_row=$db->get_one("select * from xycms_book where id=$b_id",MYSQL_ASSOC);
?>
